...
def gml2png(gml_input_path, png_output_path, label_flag=True, straight_line=False, bg_type=1, edge_weight=1.0):
    ...
    bbox = [min_x - max_r - 4, min_y - max_r - 4, max_x + max_r + 4, max_y + max_r + 4]
    ...
    for edge in graph['edges']:
        ...
        for i in range(len(points) - 1):
            draw.line([points[i], points[i + 1]], fill='#' + edge['color'], width=int(edge['value']*edge_weight))
