...
if __name__=="__main__":
    ...
    for pp_id in os.listdir('../temp_files/attributed_idea_tree_by_year/'):
        if os.path.isfile(f'../temp_files/attributed_idea_tree_by_year/{pp_id}/{str(datetime.datetime.now().year)}.gml'):
            finished_pids1.append(pp_id)

    finished_pids2 = []
    for pp_id in os.listdir('../temp_files/node_entropy_by_year/'):
        if os.path.isfile(f'../temp_files/node_entropy_by_year/{pp_id}/{str(datetime.datetime.now().year)}'):
            finished_pids2.append(pp_id)
    bug_pids = list(set(finished_pids2).difference(set(finished_pids1)))
    print(len(bug_pids))
    for p_id in bug_pids:
        gen_visible_depth_marked_skeleton_tree_gml_and_high_KE_node_detail(p_id, 2022)